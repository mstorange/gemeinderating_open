...
if check_password():
    ...
    with st.form("filter_form"):
        ...
        with st.expander("Filter setzen (bspw. nur alle Gemeinden mit Mietpreisen > 250 CHF/m2 einblenden)", expanded=False):
            ...
            slider_bev = st.slider(label="Bevölkerung Prognose (Verhältnis zu 2055)", min_value=data['Bevölkerung (Prognose) '].min()-10, max_value=data['Bevölkerung (Prognose) '].max()+10, step=10, value=(data['Bevölkerung (Prognose) '].min()-10, data['Bevölkerung (Prognose) '].max()+10))

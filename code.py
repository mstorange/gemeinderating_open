...
if check_password():
    ...
    with st.form("filter_form"):
        ...
        with st.expander("Filter setzen (bspw. nur alle Gemeinden mit Mietpreisen > 250 CHF/m2 einblenden)", expanded=False):
            ...
            slider_miete1 = st.slider(label="Mietzins (70%-Q.)", min_value=data['Wohnpreis (aktuell)    '].min(), max_value=data['Wohnpreis (aktuell)    '].max(), step=10, value=(data['Wohnpreis (aktuell)    '].min(), data['Wohnpreis (aktuell)    '].max()))
            slider_miete2 = st.slider(label="Mietzins-Verhältnis vgl. zu Region min (70%-Q.)", min_value=data['Wohnpreis (vgl. Region)'].min(), max_value=data['Wohnpreis (vgl. Region)'].max(), step=0.1, value=(data['Wohnpreis (vgl. Region)'].min(), data['Wohnpreis (vgl. Region)'].max()))
            slider_miete3 = st.slider(label="Mietzins-Entwicklung seit 2023 (70%-Q.)", min_value=data['Wohnpreis (Entwicklung)'].min(), max_value=data['Wohnpreis (Entwicklung)'].max(), step=0.1, value=(data['Wohnpreis (Entwicklung)'].min(), data['Wohnpreis (Entwicklung)'].max()))
            slider_land1 = st.slider(label="Baulandpreis (50%-Q.)", min_value=data['Baulandpreis (aktuell) '].min(), max_value=data['Baulandpreis (aktuell) '].max(), step=50, value=(data['Baulandpreis (aktuell) '].min(), data['Baulandpreis (aktuell) '].max()))
            slider_land2 = st.slider(label="Baulandpreis-Entwicklung (Verhältnis zu 2020, 50%-Q.)", min_value=data['Baulandpreis (Entw.)   '].min(), max_value=data['Baulandpreis (Entw.)   '].max(), step=0.1, value=(data['Baulandpreis (Entw.)   '].min(), data['Baulandpreis (Entw.)   '].max()))
            slider_bev = st.slider(label="Bevölkerung Prognose (Verhältnis zu 2055)", min_value=data['Bevölkerung (Prognose) '].min(), max_value=data['Bevölkerung (Prognose) '].max(), step=10.0, value=(data['Bevölkerung (Prognose) '].min(), data['Bevölkerung (Prognose) '].max()))
            slider_alterung = st.slider(label="Alterung Prognose (Anteil Ü50, Verhältnis zu 2045)", min_value=data['Alterung (Prognose)    '].min(), max_value=data['Alterung (Prognose)    '].max(), step=0.1, value=(data['Alterung (Prognose)    '].min(), data['Alterung (Prognose)    '].max()))
            slider_beschäftigte = st.slider(label="Beschäftigte Prognose (Verhältnis zu 2050)", min_value=data['Beschäftigte (Prognose)'].min(), max_value=data['Beschäftigte (Prognose)'].max(), step=0.1, value=(data['Beschäftigte (Prognose)'].min(), data['Beschäftigte (Prognose)'].max()))
            slider_err_öv = st.slider(label="Erreichbarkeit ÖV (50 Min.-Umkreis)", min_value=data['Erreichbarkeit ÖV      '].min(), max_value=data['Erreichbarkeit ÖV      '].max(), step=10000, value=(data['Erreichbarkeit ÖV      '].min(), data['Erreichbarkeit ÖV      '].max()))
            slider_err_miv = st.slider(label="Erreichbarkeit MIV (50 Min.-Umkreis)", min_value=data['Erreichbarkeit MIV     '].min(), max_value=data['Erreichbarkeit MIV     '].max(), step=100000, value=(data['Erreichbarkeit MIV     '].min(), data['Erreichbarkeit MIV     '].max()))
            slider_steuern = st.slider(label="Steuern DINKs (Mittelwert über alle Einkommensklassen)", min_value=data['Steuern_DINKs          '].min(), max_value=data['Steuern_DINKs          '].max(), step=0.05, value=(data['Steuern_DINKs          '].min(), data['Steuern_DINKs          '].max()))
            slider_innen = st.slider(label="Innenentwicklungspotenzial (Sotomo/Urbanistica)", min_value=data['Innenentwicklungspotenzial'].min(), max_value=data['Innenentwicklungspotenzial'].max(), step=0.1, value=(data['Innenentwicklungspotenzial'].min(), data['Innenentwicklungspotenzial'].max()))
